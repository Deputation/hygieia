...
#define CPUID_EAX_FULLY_ASSOCIATIVE_CACHE(_) (((_) >> 9) & 0x01)
       ...
       */
      uint32_t
          max_addressable_ids_for_logical_processors_sharing_this_cache : 12;
...
#define CPUID_EAX_VERSION_ID_OF_ARCHITECTURAL_PERFORMANCE_MONITORING(_) \
  (((_) >> 0) & 0xFF)
       ...
       */
      uint32_t
          number_of_performance_monitoring_counter_per_logical_processor : 8;
...
typedef struct {
  /**
   * EN_PMC(n). Enable bitmask. Only the first n-1 bits are valid. Bits 31:n are
   * reserved.
   *
   * @remarks If CPUID.0AH: EAX[15:8] > n
   */
  ...
  /**
   * EN_FIXED_CTR(n). Enable bitmask. Only the first n-1 bits are valid. Bits
   * 31:n are reserved.
   *
   * @remarks If CPUID.0AH: EDX[4:0] > n
   */
